...
void ggml_linear(
    ...
    if (out_dim <= 0 || in_dim <= 0 ||
        ...
        input.size() != static_cast<size_t>(in_dim) ||
    ...
    ggml_tensor * x = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, in_dim);
    ...
    output.assign(ggml_get_data_f32(y), ggml_get_data_f32(y) + out_dim);
    ggml_free(ctx);
}
