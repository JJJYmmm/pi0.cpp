...
bool Pi0LanguagePrefix::prefill_layers_batch(
    ...
    ggml_init_params params = runner.init_params(context_size, this, static_cast<uint64_t>(batch));
    ggml_context * ctx = ggml_init(params);
    if (ctx == nullptr) {
        throw std::runtime_error("failed to initialize ggml context");
    }
    ...
    for (int layer = 0; layer < layers; ++layer) {
        ...
        require_vector_shape(*input_norm_w, width, "language prefix input norm");
        require_vector_shape(*post_norm_w, width, "language prefix post attention norm");
        ...
        require_weight_shape(*gate_w, width, mlp_width, "language prefix gate_proj");
        require_weight_shape(*up_w, width, mlp_width, "language prefix up_proj");
        require_weight_shape(*down_w, mlp_width, width, "language prefix down_proj");
        ...
        ggml_tensor * ow = runner.new_weight_2d(ctx, *out_w);
        ...
        ggml_tensor * q = ggml_cont_2d(ctx, ggml_mul_mat(ctx, qw, normed), q_out, batch);
        ggml_tensor * k = ggml_cont_2d(ctx, ggml_mul_mat(ctx, kw, normed), kv_out, batch);
        ggml_tensor * v = ggml_cont_2d(ctx, ggml_mul_mat(ctx, vw, normed), kv_out, batch);
        ...
        ggml_tensor * attn_out = ggml_mul_mat(ctx, ow, attn_values);
        ...
        ggml_tensor * gate_out = ggml_gelu(ctx, ggml_mul_mat(ctx, gate, post_norm));
        ggml_tensor * up_out = ggml_mul_mat(ctx, up, post_norm);
        ggml_tensor * mlp_out = ggml_mul_mat(ctx, down, ggml_mul(ctx, gate_out, up_out));
    ...
    if (need_output) {
        ...
        require_vector_shape(*final_norm_w, width, "language prefix final norm");
        ggml_tensor * final_scale_tensor = runner.new_weight_1d_plus_one(ctx, *final_norm_w);
        y = ggml_mul(ctx, ggml_rms_norm(ctx, hidden, 1.0e-6f), final_scale_tensor);
    }
...
bool Pi0LanguagePrefix::prefill_layer_batch(
    ...
    ggml_tensor * normed = ggml_mul(ctx, ggml_rms_norm(ctx, x, 1.0e-6f), input_scale_tensor);
    ggml_tensor * q = ggml_mul_mat(ctx, qw, normed);
    ggml_tensor * k = ggml_mul_mat(ctx, kw, normed);
    ggml_tensor * v = ggml_mul_mat(ctx, vw, normed);
    ...
    ggml_tensor * mlp_out = ggml_mul_mat(ctx, down, hidden_out);
