...
int Pi0VisionMtmd::output_width() const {
    return ctx_ == nullptr ? 0 : clip_n_mmproj_embd(ctx_);
}
...
bool Pi0VisionMtmd::encode(
    ...
    for (const ImageTensor & image : images) {
        ...
        const size_t element_count = nbytes / sizeof(float);
        if (element_count % static_cast<size_t>(width) != 0) {
            return false;
        }
        ...
        out_token_count += static_cast<int>(element_count / static_cast<size_t>(width));
    }
