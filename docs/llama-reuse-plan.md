...
- `src/models/mtmd_bridge.cpp` includes and links the public `mtmd` API through
  ...
  params at runtime.
- `src/models/ggml_bridge.cpp` wraps narrow F32 `ggml_mul_mat + ggml_add`
  graphs for single and batched linear layers; the pi0/pi0.5 restricted
  action-head forward path now runs the action horizon through this bridge
  instead of a local matmul loop.
