...
| Model forward | `src/models/pi0.cpp` implements mock/tiny velocity forward, restricted pi0 state/action-head forward, and restricted pi0.5 action/time-head forward. The restricted action-head linear layers now execute the action horizon through an internal batched `ggml` graph bridge. Full SigLIP/PaliGemma/Gemma backbone is not implemented. | Partial |
...
| llama.cpp/ggml reuse | `third_party/llama.cpp` is now a required submodule gitlink; default CMake configures and links `ggml`, `llama`, and `mtmd` for `vlacpp`; `vlacpp-mtmd-bridge` verifies a real `mtmd` API call through the runtime target; `vlacpp-ggml-bridge` verifies a real `ggml` graph compute and the restricted action-head forward path uses that bridge; GGUF conversion uses llama.cpp `gguf-py`. Full OpenPI graph wiring still remains. | Partial |
...
- `vlacpp-ggml-bridge`: internal runtime bridge computes single and batched
  linear layers through `ggml_mul_mat`, `ggml_add`, and
