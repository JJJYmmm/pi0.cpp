...
| Model forward | `src/models/pi0.cpp` implements mock/tiny velocity forward, restricted pi0 state/action-head forward, and restricted pi0.5 action/time-head forward. The restricted action-head linear layers now execute through an internal `ggml` graph bridge. Full SigLIP/PaliGemma/Gemma backbone is not implemented. | Partial |
...
- `vlacpp-ggml-bridge`: internal runtime bridge computes a linear layer through
  `ggml_mul_mat`, `ggml_add`, and `ggml_graph_compute_with_ctx`.
