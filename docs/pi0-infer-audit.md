...
| llama.cpp/ggml reuse | `third_party/llama.cpp` is now a required submodule gitlink; default CMake configures and links `ggml`, `llama`, and `mtmd` for `vlacpp`; `vlacpp-mtmd-bridge` verifies a real `mtmd` API call through the runtime target; GGUF conversion uses llama.cpp `gguf-py`. Full OpenPI graph wiring still remains. | Partial |
